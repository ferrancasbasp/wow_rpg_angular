...
export class CharacterService {
  ...
  valkyrieApplyFlight(duration: number): string {
    ...
    let text = '';
    if (ifnRank > 0) {
      const energy = this.character().level * ifnRank;
      this.addSpearCharge(energy);
      this.addShieldCharge(energy);
      text = ' · ⚔️🛡️ lanza y escudo +' + energy;
    }
  ...
  readonly unlockedUtility = computed<any[]>(() => {
    ...
    return this.classConfig().abilities.filter(a => a.type === 'utility' && !a.petAbility && (a.capstoneGate ? this.selectedCapstone() === a.capstoneGate : a.passive ? this.character().level >= a.requiredLevel : this.trainedRank(a.id) > 0)).map(a => {
      ...
      return {
        ...
        healthCostPct: a.id === 'bloodrage' ? Math.max(0, (a.healthCostPct || 0.15) - this.talentRank('improved_bloodrage') * 0.075) : a.healthCostPct,
